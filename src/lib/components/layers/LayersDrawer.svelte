<script lang="ts">
	...
	function toggleLayerVisibility(layerId: string) {
		const currentVisibility = $dynamicLayerVisibilities.get(layerId) !== false;
		mapStore.setDynamicLayerVisibility(layerId, !currentVisibility);
	}
				...
				{#each activeLayersList as layer (layer.id)}
					<div class="layer-item" animate:flip={{ duration: 200 }}>
						...
						<button
							...
							class:visible={$dynamicLayerVisibilities.get(layer.id) !== false}
							onclick={() => toggleLayerVisibility(layer.id)}
							aria-label="Toggle layer visibility"
						>
							{#if $dynamicLayerVisibilities.get(layer.id) !== false}
								<svg
									width="16"
									height="16"
									viewBox="0 0 24 24"
